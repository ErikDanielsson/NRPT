...
function make_config(problem, order::Union{Int, Nothing})
    ...
    ptproblem = PathProblem(problem, path, SliceSampler(0.1, 1))
...
directory() = joinpath(@__DIR__, "normal-results-0.1-single-dimensional-small-step")
...
function show_results(runs)
    ...
    title = "Normal interpolation, small stepsize"

    p = compare_barriers(title, runs, colors, name_map)
    println(p)
    save(joinpath(dir, "barriers.pdf"), p)

    # p = compare_barriers(runs, colors, name_map; window_size =0)
    # save(joinpath(dir, "barriers-smoothed-10.pdf"), p)
