...
function make_path(linear)
    if linear 
        ...
        return PowerPath(1.0, AutoForwardDiff())
    end
end

function make_path_problem(problem, path)
    PathProblem(problem, path, NormalIIDExplorer())
end
...
function run_problem!(runs, name, opt, pt_config, problem_config)
    # Run the same problem with NRPT
    opt_target, opt_state = opt
    ...
    runs[name] = optimized_nrpt(
        ...
        objective=opt_target
    )
end


function setup_optimizers(total_config)
    optimizers = make_optimizers(total_config.optimizer_config; targets=total_config.target_config) 
...
function run(config; runs=Dict(), run_fn="run.jld2", config_fn="config.jld2")
    ...
    for (name, opt) in optimizers
        ...
        if !(name in keys(runs))
            ...
            run_problem!(runs, name, opt, config.pt_config, config.problem_config)
        else
