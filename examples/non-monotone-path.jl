...
function make_path(name)
    if name == "linear" 
        return LinearPath()
    elseif name == "non-monotone" 
        return NonMonotonePath()
    elseif name == "mixture" 
        return QPath(0.0, AutoForwardDiff())
    else
        error("no path")
...
function run_problem!(runs, name, pt_config, problem_config)
    # Run the same problem with NRPT
    ptproblem = make_pt_problem(name)
    # optimizer = make_optimizer(opt_state, problem_config.n_knots)
    optimizer = NoOptState()
...
function run(config; runs=Dict(), run_fn="run.jld2", config_fn="config.jld2")
    ...
    for name in config.path_config
        if !(name in keys(runs))
            @info "Running NRPT with $name optimizer"
            run_problem!(runs, name, config.pt_config, config.problem_config)
        else
