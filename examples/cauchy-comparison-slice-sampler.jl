...
function make_path(n_knots)
   return SplinePath(n_knots, AutoForwardDiff())
end

function make_path_problem(problem, path)
    PathProblem(problem, path, SliceSampler())
end
...
directory(config) = "cauchy-knots:$(config.problem_config.n_knots)"
