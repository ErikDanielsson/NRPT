...
function make_path(problem, n_knots)
   return PowerPath(1., 1., problem, AutoForwardDiff())
end
...
function make_prox_opt()
    return ProjectionState(Box(1e-8, 100.))
end

function make_optimizer(optimizer)
    return (typeof(optimizer) != NoOptState
        ? ProximalStochOptState(optimizer, make_prox_opt())
        : optimizer)
end
