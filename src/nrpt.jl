...
function run_opt_round!(
    ...
    obj_val = adapt_path!(problem, chains, opt_state, objective, threaded)
    Λ_opt = compute_Λ(r, schedule; use_accept=false)
    record!(loss_recorder, Λ_opt, obj_val)

    return obj_val, Λ_opt
end
...
function optimized_nrpt(config::NRPTConfig{T, S}) where {T, S}
    ...
    opt_iters      = [config.steps_per_round(n) for n in 1:config.n_rounds
                      if config.steps_per_round(n) >= config.min_step_for_opt]
    n_opt_rounds   = length(opt_iters)
    all_iterations = build_all_iterations(config.n_rounds, config.min_step_for_opt, config.steps_per_round)
    ...
    opt_cum_iters = 0
    opt_round_n = 0
    prog_beta = Progress(sum(schedule_iters); desc="Schedule optimization")
    prog_opt = Progress(sum(opt_iters); desc="Path optimization", offset=2)
    ProgressMeter.update!(prog_beta, beta_cum_iters, force=true)
    ProgressMeter.update!(prog_opt, opt_cum_iters, force=true, showvalues=[
        ("objective", nothing),
        ("Λ", nothing),
        ("η", nothing),
    ...
    for n in 1:config.n_rounds
        ...
        if schedule_iters[n] >= config.min_step_for_opt
            opt_round_n += 1
            iterations = opt_iters[opt_round_n]
            refresh_chains!(chains, schedule, iterations)
            obj_val, Λ_opt = run_opt_round!(
                chains, config.problem, opt_state, config.objective,
                config.threaded, opt_round_n, index_process, sample_recorder,
                loss_recorder, logz_recorder, lp_recorder
            )
    ...
    # ProgressMeter.update!(prog, total_iters, force=true)
