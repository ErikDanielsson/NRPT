...
function (loss::ISBarrierLoss{P, C, S, R, Val{false}})(t) where {P <: ParametrizedPath, C, S, R}
    ...
    log_w = [
        sum(
                loss.path(t, chains[n].base_potentials[:, i], schedule[n]) - loss.ref_lps[n][i]
                for n in 1:N
            )
            for i in 1:M
    ]
...
function _min_ess_barrier(path, t, chains, schedule, ref_lps)
    ...
    log_w = [
        sum(
                path(t, chains[n].base_potentials[:, i], schedule[n]) - ref_lps[n][i]
                for n in 1:N
            )
            for i in 1:M
    ]
...
NewtonTrustRegionBarrierState(backend::AbstractADType; δ = 0.9, max_steps = 20, λ_reg = 1.0e-6) =
    NewtonTrustRegionBarrierState(NoProx(), backend, Float64(δ), max_steps, Float64(λ_reg), Int[], Float64[], [])

NewtonTrustRegionBarrierState(prox::ProximalState, backend::AbstractADType; δ = 0.9, max_steps = 20, λ_reg = 1.0e-6) =
