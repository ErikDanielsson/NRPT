...
function init(problem::PathProblem{ParametrizedPath{P}, E}, state::ScaledAdaGradState{Nothing}) where {P, E}
    return ScaledAdaGradState(state.eta, state.eps, problem.path.params, state.scaler)
end
